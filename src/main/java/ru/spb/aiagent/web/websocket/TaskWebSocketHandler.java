...
public class TaskWebSocketHandler implements io.vertx.core.Handler<RoutingContext> {
    ...
    public void handle(RoutingContext ctx) {
        ...
        ctx.request().toWebSocket().onSuccess(ws -> bind(ws, resolvedClientId)).onFailure(ctx::fail);
    }
        ...
        ws.textMessageHandler(raw -> {
            try {
                ...
                switch (message.action()) {
                    ...
                    }).onFailure(error -> registry.send(ws, Map.of("type", "ERROR", "message", error.getMessage())));
                    case "UNSUBSCRIBE" -> {
                    ...
                    default -> registry.send(ws, Map.of("type", "ERROR", "message", "Unknown action"));
                }
            } catch (Exception e) {
