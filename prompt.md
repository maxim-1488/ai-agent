## Prompt 0
Изучи файл с тестовым задание и составь задачу на реализацию проекта Агента ИИ Java 21 + Vert.x и создай файл task.md
...
## Prompt 11
Улучши внешний вид frontend без изменения backend-контрактов, REST API, WebSocket protocol, бизнес-логики задач и схемы PostgreSQL.

Цель: сделать интерфейс ближе к современному chat UI в духе chatgpt.com: спокойный светло-серый фон, менее тяжёлый sidebar, аккуратная строка создания нового чата и текстовый индикатор выполнения вместо окна с процентным прогрессом.

Изучи текущую frontend-структуру перед изменениями: `frontend/src/components/layout/Sidebar.tsx`, `frontend/src/components/chat/TaskProgress.tsx`, `frontend/src/components/chat/AgentMessage.tsx`, `frontend/src/components/chat/ChatView.tsx`, `frontend/src/style.css`, frontend tests и build-конфигурацию. Не трогай backend-код, если для этой задачи это не требуется.

Требования к sidebar:

1. Заголовок sidebar сделать визуально современнее: использовать качественный системный стек шрифтов, например `SF Pro Rounded`, `Avenir Next`, `Inter`, `ui-rounded`, `system-ui`, без подключения внешних CDN или сетевых зависимостей.
2. Заголовок должен выглядеть как брендовый элемент: аккуратный вес, плотный letter-spacing, современная графика текста допустима через CSS gradient text.
3. Кнопку `Новая задача` переименовать в `Новый чат`.
4. Оформить `Новый чат` не как тяжёлую primary-кнопку, а как кликабельную строку в sidebar, близкую по поведению к chatgpt.com.
5. Добавить рядом с `Новый чат` иконку. Предпочтительно inline SVG, чтобы не добавлять новую dependency.
6. Сохранить keyboard accessibility: элемент должен оставаться `<button type="button">`, иметь понятную область клика и hover state.

Требования к индикатору выполнения:

1. Убрать отдельное окно/карточку поверх интерфейса, процентное значение и progress bar из chat progress UI.
2. Не показывать пользователю проценты выполнения (`42%`, `77%`, `100%` и т.п.) в активном сообщении агента.
3. Вместо процентов показывать короткие живые текстовые статусы с многоточием, например: `Ищу детали…`, `Собираю информацию…`, `Считаю байты…`, `Сверяю факты…`, `Формирую ответ…`.
4. Для статуса `CREATED` использовать более подходящие фразы ожидания, например: `Готовлю контекст…`, `Жду запуск…`, `Настраиваю агента…`.
5. Текст должен периодически меняться локально на frontend, чтобы UI не выглядел зависшим между WebSocket-событиями.
6. Добавить лёгкую анимацию точек рядом с текстом через CSS, без тяжёлых компонентов и без новых библиотек.
7. Сохранить кнопку отмены задачи и текущую логику `canCancel`.
8. Не менять модель `Task.progress` и обработку WebSocket-событий: backend может продолжать присылать progress, но frontend не должен отображать его как проценты в chat progress UI.

Требования к цветовой схеме:

1. Фон всего приложения перевести в спокойные светло-серые тона.
2. Sidebar сделать светло-серым, с мягким градиентом или тонким отличием от основного фона.
3. Сообщения пользователя, composer, hover states и границы привести к единой нейтральной палитре.
4. Не ухудшать контраст текста и интерактивных элементов.
5. Не добавлять тёмную тему в рамках этой задачи.

Тесты и сборка:

1. Обновить frontend tests под новый UX: старые ожидания процентов и текста `Агент выполняет задачу`/`Агент ожидает запуска` заменить на проверки новых текстовых статусов.
2. Добавить или сохранить проверки, что проценты больше не отображаются в активном progress UI.
3. Выполнить frontend tests.
4. Выполнить production build frontend.
5. Если проект хранит `frontend/dist` в репозитории, пересобрать dist и оставить его согласованным с source-файлами.
6. Не коммитить изменения самостоятельно.

Границы изменений:

1. Не менять REST contracts, WebSocket protocol, PostgreSQL schema, backend state machine, AI execution и client isolation.
2. Не добавлять новые npm dependencies ради иконки, шрифта или анимации.
3. Не подключать внешние шрифты через интернет.
4. Не делать большой редизайн вне перечисленных областей.
5. Сохранять существующую адаптивность sidebar и composer.

В конце показать:

1. какие frontend-файлы изменены;
2. как изменился sidebar;
3. как теперь отображается выполнение запроса;
4. почему проценты больше не показываются пользователю;
